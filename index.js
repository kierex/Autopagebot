...
// Create storage folder for JSON files
...
function getAllCommands() {
    ...
    for (const file of files) {
        try {
            ...
            if (Array.isArray(cmd.name)) { cmdNames = cmd.name; primaryName = cmd.name[0]; }
            else if (typeof cmd.name === 'string') { cmdNames = [cmd.name]; primaryName = cmd.name; }
            else if (cmd.name) { cmdNames = [String(cmd.name)]; primaryName = String(cmd.name); }
...
function checkCooldown(commandName, senderId) {
    ...
    if (remaining <= 0) { cooldowns.delete(key); return { onCooldown: false, remaining: 0 }; }
...
function setCooldown(commandName, senderId, seconds) {
    ...
    const key = `${commandName}_${senderId}`;
    cooldowns.set(key, { expires: Date.now() + (seconds * 1000), command: commandName, userId: senderId });
    setTimeout(() => { if (cooldowns.get(key)?.expires <= Date.now()) cooldowns.delete(key); }, seconds * 1000);
}
...
app.get('/api/sessions', async (req, res) => {
    try {
        ...
        const sessionsWithDetails = sessions.map(s => ({
            id: s.id, name: s.name, username: s.username, owner: s.owner, 
            connectedAt: s.connectedAt, lastActive: s.lastActive, messengerLink: s.messengerLink,
...
app.get('/api/commands', (req, res) => {
    try {
        const commands = getAllCommands();
        res.json({ commands: commands, count: commands.length, categories: [...new Set(commands.map(c => c.category))], cooldownRange: { min: 0, max: 20 } });
    } catch (error) {
...
app.post('/api/connect', async (req, res) => {
    ...
    try {
        ...
        await tokenManager.addToken(pageId, {
            token: pageToken, name: name, username: username, owner: userName || 'Anonymous',
            connectedAt: new Date().toISOString(), lastActive: new Date().toISOString(),
            sessionId: req.sessionID, ip: req.ip || req.connection.remoteAddress, userAgent: req.headers['user-agent']
        });
...
app.post('/webhook', async (req, res) => {
    ...
    for (const entry of req.body.entry || []) {
        ...
        const tokenData = await tokenManager.getToken(pageId);
        if (!tokenData) { console.log(`❌ No token found for page ${pageId}`); continue; }
        await tokenManager.updateLastActive(pageId);
        for (const event of entry.messaging || []) {
            try {
                incrementMessageCount();
                if (event.message) await handleMessage(event, tokenData.token, pageId);
                else if (event.postback) await handlePostback(event, tokenData.token, pageId);
            } catch (error) {
