    ...
    async execute(senderId, args, pageAccessToken, event, sendMessageFunc, imageCache) {
        ...
        if (!args.length) {
            return sendMessage(senderId, { 
                text: '🤖 Please provide a question.\n\n📝 Usage: copilot what is the meaning of life?\n\n✨ Example: copilot tell me a joke' 
            }, pageAccessToken);
        }

        const header = '💬 | 𝗖𝗼𝗽𝗶𝗹𝗼𝘁 𝗔𝗜\n・────────────・\n';
        const footer = '\n・────────────・';

        try {
            const response = await axios.get('https://yin-api.vercel.app/ai/copilot', {
                params: { 
                    message: message,
                    model: 'gpt-5'
                }
